...
use {
    ...
    trezoa_stake_interface::stake_history::Epoch,
    std::num::NonZeroU32,
};
...
pub enum StakePoolInstruction {
    ...
    ///     trezoa_stake_interface::tools::get_minimum_delegation()
    ///   )`.
    ...
    /// trezoa_stake_interface::tools::get_minimum_delegation())`.   plus the
    /// rent-exempt amount.
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[s]` Staker
    ...
    /// between TRZ staked on different validators, the staker can force all
    ...
    ///
    ///   Succeeds if the stake account has enough TRZ to cover the desired
    ...
    ///   Deposit TRZ directly into the pool's reserve account. The output is a
    ///   "pool" token representing ownership into the pool. Inputs are
    ///   converted to the current ratio.
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[]` Stake pool withdraw authority
    ///   2. `[w]` Reserve stake account, to deposit TRZ
    ...
    ///   9. `[]` Token program id
    ///  10. `[s]` (Optional) Stake pool trz deposit authority.
    DepositSol(u64),

    ///  (Manager only) Update TRZ deposit, stake deposit, or TRZ withdrawal
    /// authority.
    ///
    ///  0. `[w]` Stake pool
    ///  1. `[s]` Manager
    ...
    ///   Withdraw TRZ directly from the pool's reserve account. Fails if the
    ///   reserve does not have enough TRZ.
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[]` Stake pool withdraw authority
    ///   2. `[s]` User transfer authority, for pool token account
    ///   3. `[w]` User account to burn pool tokens
    ///   4. `[w]` Reserve stake account, to withdraw TRZ
    ...
    ///  11. `[]` Token program id
    ///  12. `[s]` (Optional) Stake pool trz withdraw authority
    WithdrawSol(u64),

    /// Create token metadata for the stake-pool token in the
    /// trezoaplex-token program
    /// 0. `[]` Stake pool
    /// 1. `[s]` Manager
    /// 2. `[]` Stake pool withdraw authority
    ...
    CreateTokenMetadata {
        ...
        symbol: String,
        /// URI of the uploaded metadata of the tpl-token
        uri: String,
    },
    ...
    ///   Deposit TRZ directly into the pool's reserve account, with a
    ...
    ///   Withdraw TRZ directly from the pool's reserve account. Fails if the
    ///   reserve does not have enough TRZ or if the slippage constraint is not
    ///   met.
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[]` Stake pool withdraw authority
...
pub fn add_validator_to_pool(
    ...
    let accounts = vec![
        ...
        AccountMeta::new_readonly(trezoa_sdk_ids::sysvar::rent::id(), false),
        AccountMeta::new_readonly(trezoa_sdk_ids::sysvar::clock::id(), false),
        AccountMeta::new_readonly(trezoa_sdk_ids::sysvar::stake_history::id(), false),
        #[allow(deprecated)]
        AccountMeta::new_readonly(trezoa_stake_interface::config::id(), false),
        AccountMeta::new_readonly(trezoa_system_interface::program::id(), false),
        AccountMeta::new_readonly(trezoa_stake_interface::program::id(), false),
    ];
...
fn deposit_stake_internal(
    ...
    if let Some(stake_pool_deposit_authority) = stake_pool_deposit_authority {
        ...
        instructions.extend_from_slice(&[
            trezoa_stake_interface::instruction::authorize(
                deposit_stake_address,
                deposit_stake_withdraw_authority,
                stake_pool_deposit_authority,
                trezoa_stake_interface::state::StakeAuthorize::Staker,
                None,
            ),
            trezoa_stake_interface::instruction::authorize(
                deposit_stake_address,
                deposit_stake_withdraw_authority,
                stake_pool_deposit_authority,
                trezoa_stake_interface::state::StakeAuthorize::Withdrawer,
                None,
            ),
        ]);
    } else {
...
/// Creates instructions required to deposit TRZ directly into a stake pool.
...
/// Creates instruction to deposit TRZ directly into a stake pool.
pub fn deposit_sol(
    program_id: &Pubkey,
    stake_pool: &Pubkey,
    stake_pool_withdraw_authority: &Pubkey,
...
/// Creates instruction to deposit TRZ directly into a stake pool with slippage
/// constraint.
...
/// Creates instruction required to deposit TRZ directly into a stake pool.
...
/// Creates instruction required to withdraw TRZ directly from a stake pool.
pub fn withdraw_sol(
    program_id: &Pubkey,
    stake_pool: &Pubkey,
    stake_pool_withdraw_authority: &Pubkey,
...
/// Creates instruction required to withdraw TRZ directly from a stake pool with
...
/// Creates instruction required to withdraw TRZ directly from a stake pool.
/// The difference with `withdraw_sol()` is that the trz withdraw authority
...
/// Creates an instruction to update metadata in the tpl token metadata program
...
/// Creates an instruction to create metadata using the tpl token metadata
