...
use {
    ...
    trezoa_borsh::v1::get_instance_packed_len,
    trezoa_msg::msg,
    trezoa_program_error::ProgramError,
    trezoa_program_memory::sol_memcmp,
    trezoa_program_pack::{Pack, Sealed},
    trezoa_pubkey::{Pubkey, PUBKEY_BYTES},
    trezoa_stake_interface::state::Lockup,
    tpl_pod::primitives::{PodU32, PodU64},
...
pub struct StakePool {
    ...
    /// i.e. `sol_deposit_fee`% of TRZ deposited is collected as deposit fees
    /// for every deposit and `sol_referral_fee`% of the collected TRZ
...
impl StakePool {
    ...
    /// calculate pool tokens to be deducted as TRZ deposit fees
    #[inline]
    ...
    /// calculate pool tokens to be deducted from TRZ deposit fees as referral
    /// fees
    #[inline]
    ...
    pub(crate) fn check_sol_deposit_authority(
        ...
        if let Some(auth) = self.sol_deposit_authority {
            ...
            if !sol_deposit_authority.is_signer {
                msg!("TRZ Deposit authority signature missing");
    ...
    /// Checks that the trz withdraw authority is valid
    ...
    pub(crate) fn check_sol_withdraw_authority(
        ...
        if let Some(auth) = self.sol_withdraw_authority {
            ...
            if !sol_withdraw_authority.is_signer {
                msg!("TRZ withdraw authority signature missing");
                return Err(StakePoolError::SignatureMissing.into());
            }
        }
        Ok(())
...
mod test {
    ...
    use {
        super::*,
        proptest::prelude::*,
        trezoa_borsh::v1::{get_packed_len, try_from_slice_unchecked},
        trezoa_clock::{DEFAULT_SLOTS_PER_EPOCH, DEFAULT_S_PER_SLOT, SECONDS_PER_DAY},
    ...
    fn specific_fee_calculation() {
        // 10% of 10 TRZ in rewards should be 1 TRZ in fees
        let epoch_fee = Fee {
            numerator: 1,
            denominator: 10,
        };
