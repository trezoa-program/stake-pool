...
use {
    ...
    tpl_token_2022_interface::extension::ExtensionType,
    test_case::test_case,
};
...
#[test_case(tpl_token_interface::id(); "token")]
#[test_case(tpl_token_2022_interface::id(); "token-2022")]
#[tokio::test]
...
async fn fail_with_freeze_authority() {
    ...
    let wrong_mint = Keypair::new();
    let rent = banks_client.get_rent().await.unwrap();
    let mint_rent = rent.minimum_balance(tpl_token_interface::state::Mint::LEN);
...
async fn success_with_supported_extensions() {
    ...
    let stake_pool_accounts =
        StakePoolAccounts::new_with_token_program(tpl_token_2022_interface::id());
...
async fn fail_with_fee_owned_by_wrong_token_program_id() {
    ...
    let account_rent = rent.minimum_balance(tpl_token_interface::state::Account::LEN);
    let transaction = Transaction::new_signed_with_payer(
        &[system_instruction::create_account(
            &payer.pubkey(),
            &stake_pool_accounts.pool_fee_account.pubkey(),
