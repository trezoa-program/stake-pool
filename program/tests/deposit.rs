...
use {
    ...
    tpl_stake_pool::{error::StakePoolError, id, instruction, state, MINIMUM_RESERVE_LAMPORTS},
    tpl_token_interface::error as token_error,
    test_case::test_case,
};
...
async fn success(token_program_id: Pubkey) {
    ...
    // Check validator stake account actual TRZ balance
...
async fn success_with_extra_stake_lamports() {
    let (
        ...
        pool_token_account,
        stake_lamports,
    ) = setup(tpl_token_interface::id()).await;
