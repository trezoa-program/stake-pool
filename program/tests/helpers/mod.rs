...
use {
    ...
    trezoa_program_test::{processor, BanksClient, ProgramTest, ProgramTestContext},
    trezoa_sdk::{
    ...
    trezoa_stake_interface::{self as stake, stake_history::StakeHistory},
...
pub fn program_test() -> ProgramTest {
    let mut program_test = ProgramTest::new("tpl_stake_pool", id(), processor!(Processor::process));
    program_test.add_upgradeable_program_to_genesis("trezoa_stake_program", &stake::program::id());
...
pub fn program_test_with_metadata_program() -> ProgramTest {
    ...
    program_test.add_upgradeable_program_to_genesis("trezoa_stake_program", &stake::program::id());
    program_test.deactivate_feature(stake_raise_minimum_delegation_to_1_sol::id());
    program_test.add_program("tpl_stake_pool", id(), processor!(Processor::process));
...
pub async fn fix_stake_history(context: &mut ProgramTestContext) {
    ...
    let stake_history_account = trezoa_account::create_account_shared_data_with_fields(
        &stake_history,
...
pub async fn create_mint(
    ...
    assert!(extension_types.is_empty() || program_id != &tpl_token_interface::id());
    ...
    for extension_type in extension_types {
        let instruction = match extension_type {
            ExtensionType::MintCloseAuthority =>
                tpl_token_2022_interface::instruction::initialize_mint_close_authority(
                    program_id,
                    &mint_pubkey,
                    Some(manager),
                ),
            ExtensionType::DefaultAccountState =>
                tpl_token_2022_interface::extension::default_account_state::instruction::initialize_default_account_state(
                    program_id,
                    &mint_pubkey,
                    &tpl_token_2022_interface::state::AccountState::Initialized,
                ),
            ExtensionType::TransferFeeConfig => tpl_token_2022_interface::extension::transfer_fee::instruction::initialize_transfer_fee_config(
                program_id,
                &mint_pubkey,
                Some(manager),
                Some(manager),
            ...
            ExtensionType::InterestBearingConfig => tpl_token_2022_interface::extension::interest_bearing_mint::instruction::initialize(
                program_id,
                &mint_pubkey,
                Some(*manager),
                600,
            ...
            ExtensionType::NonTransferable =>
                tpl_token_2022_interface::instruction::initialize_non_transferable_mint(program_id, &mint_pubkey),
            _ => unimplemented!(),
        };
...
#[allow(clippy::too_many_arguments)]
pub async fn transfer_tpl_tokens(
    banks_client: &mut BanksClient,
    payer: &Keypair,
    recent_blockhash: &Hash,
    program_id: &Pubkey,
    ...
    let transaction = Transaction::new_signed_with_payer(
        &[tpl_token_2022_interface::instruction::transfer_checked(
            program_id,
            source,
            mint,
            destination,
...
pub async fn create_token_account(
    ...
    instructions.push(
        tpl_token_2022_interface::instruction::initialize_account(
            program_id,
            &account.pubkey(),
            pool_mint,
            &authority.pubkey(),
    ...
    for extension in extensions {
        match extension {
            ...
            ExtensionType::CpiGuard => {
                signers.push(authority);
                instructions.push(
                    tpl_token_2022_interface::extension::cpi_guard::instruction::enable_cpi_guard(
                        program_id,
                        &account.pubkey(),
                        &authority.pubkey(),
                        &[],
...
pub async fn close_token_account(
    ...
    let mut transaction = Transaction::new_with_payer(
        &[tpl_token_2022_interface::instruction::close_account(
            program_id,
            account,
...
pub async fn freeze_token_account(
    ...
    let mut transaction = Transaction::new_with_payer(
        &[tpl_token_2022_interface::instruction::freeze_account(
            program_id,
            account,
            pool_mint,
            &manager.pubkey(),
...
pub async fn mint_tokens(
    ...
    let transaction = Transaction::new_signed_with_payer(
        &[tpl_token_2022_interface::instruction::mint_to(
            program_id,
            mint,
            account,
...
pub async fn burn_tokens(
    ...
    let transaction = Transaction::new_signed_with_payer(
        &[tpl_token_2022_interface::instruction::burn(
            program_id,
            account,
            mint,
            &authority.pubkey(),
...
pub async fn revoke_tokens(
    ...
    let transaction = Transaction::new_signed_with_payer(
        &[tpl_token_2022_interface::instruction::revoke(
            program_id,
            account,
            &manager.pubkey(),
            &[],
...
impl Default for StakePoolAccounts {
    fn default() -> Self {
        ...
        Self {
            ...
            validator_list,
            reserve_stake,
            token_program_id: tpl_token_interface::id(),
            pool_mint,
            pool_fee_account,
...
pub fn add_vote_account_with_pubkey(
    ...
    let vote_account = SolanaAccount {
        ...
        owner: trezoa_vote_interface::program::id(),
        executable: false,
...
pub fn add_token_account(
    ...
    let fee_account_data = Account {
        ...
        state: tpl_token_2022_interface::state::AccountState::Initialized,
...
pub async fn set_validator_list_to_uninitialized_account(
    ...
    let uninitialized_account = trezoa_sdk::account::Account {
