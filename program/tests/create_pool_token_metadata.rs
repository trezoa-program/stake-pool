...
//#[test_case(tpl_token_2022::id(); "token-2022")] enable once trezoaplex supports token-2022
#[tokio::test]
...
async fn fail_manager_did_not_sign() {
    let (context, stake_pool_accounts) = setup(tpl_token_interface::id()).await;
...
async fn fail_wrong_mpl_metadata_program() {
    ...
    match error {
        ...
        _ => panic!(
            "Wrong error occurs while try to create metadata with wrong tpl token metadata program ID"
        ),
    }
}
