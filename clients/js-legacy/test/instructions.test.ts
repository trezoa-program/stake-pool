...
import { TOKEN_PROGRAM_ID, TokenAccountNotFoundError } from '@trezoa/tpl-token';
  ...
  describe('depositSol', () => {
    ...
    it('should throw an error with invalid balance', async () => {
      await expect(depositSol(connection, stakePoolAddress, from, balance + 1)).rejects.toThrow(
        Error('Not enough TRZ to deposit into pool. Maximum deposit amount is 0.00001 TRZ.'),
      );
    });
