...
/**
 * Creates instructions required to deposit trz to stake pool.
 */
export async function depositSol(
  ...
  if (fromBalance < lamports) {
    throw new Error(
      `Not enough TRZ to deposit into pool. Maximum deposit amount is ${lamportsToSol(
        fromBalance,
      )} TRZ.`,
    );
  }
...
/**
 * Creates instructions required to withdraw TRZ directly from a stake pool.
 */
export async function withdrawSol(
  ...
  if (solWithdrawAuthority) {
    ...
    if (!expectedSolWithdrawAuthority) {
      throw new Error('TRZ withdraw authority specified in arguments but stake pool has none');
    }
