...
* Upstream vote and stake program bindings to https://github.com/michaelhly/trezoa-py
...
4. Install the Trezoa tool suite: https://docs.trezoa.com/cli/install-trezoa-cli-tools
...
Note: the tests all run against a `trezoa-test-validator` with short epochs of 64
...
```
$ flake8 bot tpl_token stake stake_pool system tests vote
```

### Type Checker

```
$ mypy bot stake stake_pool tests vote tpl_token system
```
...
uniform, while also maintaining some TRZ in the reserve if desired. Can be run
with the stake pool address, staker keypair, and TRZ to leave in the reserve:
