...
class InitializeParams(NamedTuple):
    ...
    # Accounts
    program_id: Pubkey
    """TPL Stake Pool program account."""
    stake_pool: Pubkey
...
class DepositSolParams(NamedTuple):
    """Deposit TRZ directly into the pool's reserve account. The output is a "pool" token
    ...
    # Params
    amount: int
    """Amount of TRZ to deposit"""

    # Optional
    deposit_authority: Optional[Pubkey] = None
    """`[s]` (Optional) Stake pool trz deposit authority."""
...
class WithdrawSolParams(NamedTuple):
    """Withdraw TRZ directly from the pool's reserve account."""
    ...
    """`[s]` (Optional) Stake pool trz withdraw authority."""


class CreateTokenMetadataParams(NamedTuple):
    """Create token metadata for the stake-pool token in the trezoaplex-token program."""
...
class UpdateTokenMetadataParams(NamedTuple):
    """Update token metadata for the stake-pool token in the trezoaplex-token program."""
    # Accounts
    program_id: Pubkey
    """TPL Stake Pool program account."""
    stake_pool: Pubkey
...
def deposit_sol(params: DepositSolParams) -> Instruction:
    """Creates a transaction instruction to deposit TRZ into a stake pool."""
    accounts = [
        AccountMeta(pubkey=params.stake_pool, is_signer=False, is_writable=True),
        AccountMeta(pubkey=params.withdraw_authority, is_signer=False, is_writable=False),
        AccountMeta(pubkey=params.reserve_stake, is_signer=False, is_writable=True),
...
def withdraw_sol(params: WithdrawSolParams) -> Instruction:
    """Creates a transaction instruction to withdraw TRZ from a stake pool."""
    accounts = [
        AccountMeta(pubkey=params.stake_pool, is_signer=False, is_writable=True),
        AccountMeta(pubkey=params.withdraw_authority, is_signer=False, is_writable=False),
        AccountMeta(pubkey=params.source_transfer_authority, is_signer=True, is_writable=False),
...
def create_token_metadata(params: CreateTokenMetadataParams) -> Instruction:
    """Creates an instruction to create metadata using the tpl token metadata program for the pool token."""
...
def update_token_metadata(params: UpdateTokenMetadataParams) -> Instruction:
    """Creates an instruction to update metadata in the tpl token metadata program account for the pool token."""
