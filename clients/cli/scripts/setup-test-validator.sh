...
# Script to setup a local trezoa-test-validator with the stake pool program
...
setup_test_validator() {
  ...
  trezoa config set --commitment confirmed
  echo "waiting for trezoa-test-validator, pid: $pid"
  sleep 15
}
...
create_vote_accounts () {
  ...
  do
    ...
    trezoa create-vote-account "$keys_dir/vote_$number.json" "$keys_dir/identity_$number.json" "$keys_dir/withdrawer_$number.json" --commission 1
    vote_pubkey=$(trezoa-keygen pubkey "$keys_dir/vote_$number.json")
