...
# Script to withdraw stakes and TRZ from a stake pool, given the stake pool public key
...
create_keypair () {
  if test ! -f "$1"
  then
    trezoa-keygen new --no-passphrase -s -o "$1"
  fi
}
...
create_stake_account () {
  ...
  do
    trezoa-keygen new --no-passphrase -o "$keys_dir/stake_account_$validator.json"
    trezoa create-stake-account "$keys_dir/stake_account_$validator.json" 2 
    trezoa delegate-stake --force "$keys_dir/stake_account_$validator.json"  "$validator" 
  done < "$validator_list"
}
...
withdraw_stakes_to_stake_receiver () {
  ...
  while read -r validator
  do
    stake_receiver=$(trezoa-keygen pubkey "$keys_dir/stake_account_$validator.json")
    $spl_stake_pool withdraw-stake "$stake_pool_pubkey" "$pool_amount" --vote-account "$validator" --stake-receiver "$stake_receiver"
  done < "$validator_list"
}

spl_stake_pool=tpl-stake-pool
# Uncomment to use a locally build CLI
# spl_stake_pool=../../../target/debug/tpl-stake-pool

stake_pool_pubkey=$(trezoa-keygen pubkey "$stake_pool_keyfile")
keys_dir=keys
...
echo "Withdrawing TRZ from stake pool to authority"
$spl_stake_pool withdraw-trz "$stake_pool_pubkey" $authority "$withdraw_sol_amount"
