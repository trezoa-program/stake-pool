...
# Script to deposit stakes and TRZ into a stake pool, given the stake pool keyfile
...
create_user_stakes () {
  ...
  do
    ...
    trezoa create-stake-account "$keys_dir/stake_$validator.json" "$sol_amount" --withdraw-authority "$authority" --stake-authority "$authority"
  done < "$validator_list"
}
...
delegate_user_stakes () {
  ...
  while read -r validator
  do
    trezoa delegate-stake --force "$keys_dir/stake_$validator.json" "$validator" --stake-authority "$authority"
  done < "$validator_list"
}
...
deposit_stakes () {
  ...
  while read -r validator
  do
    stake=$(trezoa-keygen pubkey "$keys_dir/stake_$validator.json")
