use {
    ...
    trezoa_sdk::transaction::Transaction,
...
) -> Result<tpl_token_2022_interface::state::Account, Error> {
    let account_data = rpc_client.get_account_data(token_account_address)?;
    let token_account =
        tpl_token_2022_interface::state::Account::unpack_from_slice(account_data.as_slice())
...
) -> Result<tpl_token_2022_interface::state::Mint, Error> {
    let account_data = rpc_client.get_account_data(token_mint_address)?;
    let token_mint =
        tpl_token_2022_interface::state::Mint::unpack_from_slice(account_data.as_slice())
