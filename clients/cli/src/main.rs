...
use {
    ...
    trezoa_client::rpc_client::RpcClient,
    ...
    tpl_token_2022_interface::{
        check_tpl_token_program_account, extension::StateWithExtensions, state::Mint,
    },
...
const FEES_REFERENCE: &str = "Consider setting a minimal fee. \
                              See https://tpl.trezoa.com/stake-pool/fees for more \
...
fn send_transaction_no_wait(
    config: &Config,
    transaction: Transaction,
) -> trezoa_client::client_error::Result<()> {
    if config.dry_run {
...
fn setup_mint_account(
    ...
    if let Ok(account) = mint_account_info {
        if check_tpl_token_program_account(&account.owner).is_ok() {
            if account.data.iter().any(|&x| x != 0) {
                if let Ok(mint) = StateWithExtensions::<Mint>::unpack(&account.data) {
                    if Option::from(mint.base.mint_authority) != Some(*withdraw_authority) {
                        return Err(format!(
                            "Mint account exists with the incorrect mint authority. Set mint authority with `tpl-token authorize {} mint {}",
                ...
                let instructions = vec![tpl_token_2022_interface::instruction::initialize_mint(
                    &account.owner,
                    &mint_keypair.pubkey(),
                    withdraw_authority,
                    None,
    ...
    let instructions = vec![
        system_instruction::create_account(
            ...
            tpl_token_2022_interface::state::Mint::LEN as u64,
            &tpl_token_interface::id(),
        ),
        tpl_token_2022_interface::instruction::initialize_mint(
            &tpl_token_interface::id(),
            &mint_keypair.pubkey(),
            withdraw_authority,
            None,
...
fn setup_and_initialize_validator_list_with_stake_pool(
    ...
    instructions.push(tpl_stake_pool::instruction::initialize(
        &config.stake_pool_program_id,
        &stake_pool_keypair.pubkey(),
        &config.manager.pubkey(),
...
fn command_create_pool(
    ...
    let mint_account_balance = config
        .rpc_client
        .get_minimum_balance_for_rent_exemption(tpl_token_2022_interface::state::Mint::LEN)?;
    ...
    let default_decimals = tpl_token_2022_interface::native_mint::DECIMALS;
...
fn create_token_metadata(
    ...
    let mut signers = vec![config.fee_payer.as_ref(), config.manager.as_ref()];
    let instructions = vec![tpl_stake_pool::instruction::create_token_metadata(
        &config.stake_pool_program_id,
        stake_pool_address,
        &stake_pool.manager,
...
fn update_token_metadata(
    ...
    let mut signers = vec![config.fee_payer.as_ref(), config.manager.as_ref()];
    let instructions = vec![tpl_stake_pool::instruction::update_token_metadata(
        &config.stake_pool_program_id,
        stake_pool_address,
        &stake_pool.manager,
        &stake_pool.pool_mint,
...
fn command_vsa_add(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[
            tpl_stake_pool::instruction::add_validator_to_pool_with_vote(
                &config.stake_pool_program_id,
                &stake_pool,
                stake_pool_address,
                vote_account,
...
fn command_vsa_remove(
    ...
    let instructions = vec![
        ...
        tpl_stake_pool::instruction::remove_validator_from_pool_with_vote(
            &config.stake_pool_program_id,
            &stake_pool,
            stake_pool_address,
            vote_account,
...
fn command_increase_validator_stake(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[
            tpl_stake_pool::instruction::increase_validator_stake_with_vote(
                &config.stake_pool_program_id,
                &stake_pool,
                stake_pool_address,
                vote_account,
...
fn command_decrease_validator_stake(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[
            tpl_stake_pool::instruction::decrease_validator_stake_with_vote(
                &config.stake_pool_program_id,
                &stake_pool,
                stake_pool_address,
                vote_account,
...
fn command_set_preferred_validator(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[tpl_stake_pool::instruction::set_preferred_validator(
            &config.stake_pool_program_id,
            stake_pool_address,
            &config.staker.pubkey(),
...
fn command_deposit_stake(
    ...
    let mut deposit_instructions =
        if let Some(stake_deposit_authority) = config.funding_authority.as_ref() {
            ...
            tpl_stake_pool::instruction::deposit_stake_with_authority(
                &config.stake_pool_program_id,
                stake_pool_address,
                &stake_pool.validator_list,
                &stake_deposit_authority.pubkey(),
            ...
            tpl_stake_pool::instruction::deposit_stake(
                &config.stake_pool_program_id,
                stake_pool_address,
                &stake_pool.validator_list,
                &pool_withdraw_authority,
...
fn command_deposit_sol(
    ...
    if from_balance < lamports {
        return Err(format!(
            "Not enough TRZ to deposit into pool: {}.\nMaximum deposit amount is {} TRZ.",
            Sol(lamports),
            Sol(from_balance)
        )
        .into());
    ...
    // ephemeral TRZ account just to do the transfer
    ...
    // Create the ephemeral TRZ account
    ...
    let deposit_instruction = if let Some(deposit_authority) = config.funding_authority.as_ref() {
        let expected_sol_deposit_authority = stake_pool.sol_deposit_authority.ok_or_else(|| {
            "TRZ deposit authority specified in arguments but stake pool has none".to_string()
        })?;
        ...
        tpl_stake_pool::instruction::deposit_sol_with_authority(
            &config.stake_pool_program_id,
            stake_pool_address,
            &deposit_authority.pubkey(),
            &pool_withdraw_authority,
        ...
        tpl_stake_pool::instruction::deposit_sol(
            &config.stake_pool_program_id,
            stake_pool_address,
            &pool_withdraw_authority,
            &stake_pool.reserve_stake,
...
fn command_list(
    ...
    let total_pool_tokens =
        tpl_token::amount_to_ui_amount(stake_pool.pool_token_supply, pool_mint.decimals);
...
fn command_update(
    ...
    let (mut update_list_instructions, final_instructions) = if stale_only {
        tpl_stake_pool::instruction::update_stale_stake_pool(
            &config.stake_pool_program_id,
            &stake_pool,
            &validator_list,
            stake_pool_address,
        ...
        tpl_stake_pool::instruction::update_stake_pool(
            &config.stake_pool_program_id,
            &stake_pool,
            &validator_list,
            stake_pool_address,
...
fn prepare_withdraw_accounts(
    ...
    if remaining_amount > 0 {
        return Err(format!(
            ...
            tpl_token::amount_to_ui_amount(pool_amount, pool_mint.decimals)
        )
        .into());
    }
...
fn command_withdraw_stake(
    ...
    let stake_pool = get_stake_pool(&config.rpc_client, stake_pool_address)?;
    let pool_mint = get_token_mint(&config.rpc_client, &stake_pool.pool_mint)?;
    let pool_amount = tpl_token::ui_amount_to_amount(pool_amount, pool_mint.decimals);
    ...
    if token_account.amount < pool_amount {
        return Err(format!(
            ...
            tpl_token::amount_to_ui_amount(pool_amount, pool_mint.decimals),
            tpl_token::amount_to_ui_amount(token_account.amount, pool_mint.decimals)
        )
        .into());
    }
    ...
    instructions.push(
        ...
        tpl_token_2022_interface::instruction::approve(
            &stake_pool.token_program_id,
            &pool_token_account,
    ...
    for withdraw_account in withdraw_accounts {
        ...
        if let Some(vote_address) = withdraw_account.vote_address {
            println!(
                ...
                tpl_token::amount_to_ui_amount(withdraw_account.pool_amount, pool_mint.decimals),
        ...
        instructions.push(tpl_stake_pool::instruction::withdraw_stake(
            &config.stake_pool_program_id,
            stake_pool_address,
            &stake_pool.validator_list,
            &pool_withdraw_authority,
    ...
    let withdraw_instruction = if let Some(withdraw_authority) = config.funding_authority.as_ref() {
        let expected_sol_withdraw_authority =
            stake_pool.sol_withdraw_authority.ok_or_else(|| {
                "TRZ withdraw authority specified in arguments but stake pool has none".to_string()
            })?;
        ...
        tpl_stake_pool::instruction::withdraw_sol_with_authority(
            &config.stake_pool_program_id,
            stake_pool_address,
            &withdraw_authority.pubkey(),
            &pool_withdraw_authority,
        ...
        tpl_stake_pool::instruction::withdraw_sol(
            &config.stake_pool_program_id,
            stake_pool_address,
            &pool_withdraw_authority,
            &user_transfer_authority.pubkey(),
...
fn command_set_manager(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[tpl_stake_pool::instruction::set_manager(
            &config.stake_pool_program_id,
            stake_pool_address,
            &config.manager.pubkey(),
            &new_manager_pubkey,
...
fn command_set_staker(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[tpl_stake_pool::instruction::set_staker(
            &config.stake_pool_program_id,
            stake_pool_address,
            &config.manager.pubkey(),
            new_staker,
...
fn command_set_funding_authority(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[tpl_stake_pool::instruction::set_funding_authority(
            &config.stake_pool_program_id,
            stake_pool_address,
            &config.manager.pubkey(),
...
fn command_set_fee(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[tpl_stake_pool::instruction::set_fee(
            &config.stake_pool_program_id,
            stake_pool_address,
            &config.manager.pubkey(),
            new_fee,
...
fn main() {
    ...
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("program_id")
                ...
                .takes_value(true)
                .help("Stake pool program id. [default: tpl_stake_pool::id() or tpl_stake_pool::devnet::id(), depending on target network]")
                .global(true)
        )
    ...
    .arg(
        ...
        .subcommand(SubCommand::with_name("increase-validator-stake")
            ...
            .arg(
                Arg::with_name("amount")
                    ...
                    .value_name("AMOUNT")
                    .takes_value(true)
                    .help("Amount in TRZ to add to the validator stake account. Must be at least the rent-exempt amount for a stake plus 1 TRZ for merging."),
            )
        )
        .subcommand(SubCommand::with_name("decrease-validator-stake")
            ...
            .arg(
                Arg::with_name("amount")
                    ...
                    .value_name("AMOUNT")
                    .takes_value(true)
                    .help("Amount in TRZ to remove from the validator stake account. Must be at least the rent-exempt amount for a stake."),
            )
        )
        ...
        .subcommand(SubCommand::with_name("deposit-trz")
            .about("Deposit TRZ into the stake pool in exchange for pool tokens")
            .arg(
                Arg::with_name("pool")
                    .index(1)
                    .validator(is_pubkey)
                ...
                Arg::with_name("amount")
                    ...
                    .value_name("AMOUNT")
                    .takes_value(true)
                    .help("Amount in TRZ to deposit into the stake pool reserve account."),
            )
            .arg(
                Arg::with_name("from")
                    .long("from")
        ...
        .subcommand(SubCommand::with_name("withdraw-trz")
            .about("Withdraw TRZ from the stake pool's reserve in exchange for pool tokens")
            .arg(
                Arg::with_name("pool")
                    .index(1)
                    .validator(is_pubkey)
            ...
            .arg(
                Arg::with_name("sol_receiver")
                    ...
                    .takes_value(true)
                    .required(true)
                    .help("System account to receive TRZ from the stake pool. Defaults to the payer."),
            )
            .arg(
                Arg::with_name("amount")
                    .index(3)
                    ...
                    .takes_value(true)
                    .required(true)
                    .help("Amount of pool tokens to withdraw for TRZ."),
            )
            .arg(
                Arg::with_name("pool_account")
                    .long("pool-account")
        ...
        .subcommand(SubCommand::with_name("set-funding-authority")
            ...
            .arg(
                Arg::with_name("funding_type")
                    ...
                    .possible_values(&["stake-deposit", "trz-deposit", "trz-withdraw"]) // FundingType enum
                    .takes_value(true)
                    .required(true)
        ...
        .subcommand(SubCommand::with_name("set-fee")
            .about("Change the [epoch/withdraw/stake deposit/trz deposit] fee assessed by the stake pool. Must be signed by the manager.")
            .arg(
                Arg::with_name("pool")
                    .index(1)
                    .validator(is_pubkey)
            ...
            .arg(Arg::with_name("fee_type")
                .index(2)
                .value_name("FEE_TYPE")
                .possible_values(&["epoch", "stake-deposit", "trz-deposit", "stake-withdrawal", "trz-withdrawal"]) // FeeType enum
                .takes_value(true)
                .required(true)
        ...
        .subcommand(SubCommand::with_name("set-referral-fee")
            ...
            .arg(Arg::with_name("fee_type")
                .index(2)
                .value_name("FEE_TYPE")
                .possible_values(&["stake", "trz"]) // FeeType enum, kind of
                .takes_value(true)
                .required(true)
                .help("Fee type to be updated."),
            )
    ...
    let _ = match matches.subcommand() {
        ...
        ("set-funding-authority", Some(arg_matches)) => {
            ...
            let funding_type = match arg_matches.value_of("funding_type").unwrap() {
                "trz-deposit" => FundingType::SolDeposit,
                "stake-deposit" => FundingType::StakeDeposit,
                "trz-withdraw" => FundingType::SolWithdraw,
                _ => unreachable!(),
            };
            let _unset = arg_matches.is_present("unset");
        ...
        ("set-fee", Some(arg_matches)) => {
            ...
            match arg_matches.value_of("fee_type").unwrap() {
                ...
                "trz-withdrawal" => command_set_fee(
                    &config,
                    &stake_pool_address,
